...
import { getServicePackageEs, SERVICE_PACKAGES_ES } from "../servicePackagesEs";

export function generateStaticParams() {
  return SERVICE_PACKAGES_ES.map((servicePackage) => ({ slug: servicePackage.slug }));
}

export default async function EsServicePackageDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const servicePackage = getServicePackageEs(slug);
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-12">
        <header className="mb-8 flex items-center justify-between">
          <Link href="/es/services" className="text-sm text-white/45 transition hover:text-white">
            ← Volver a servicios
          </Link>
        </header>
        ...
        <section className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          <div>
            ...
            <p className="mt-8 max-w-2xl text-lg leading-8 text-white/72">{servicePackage.detailTagline}</p>
          ...
          <div className="rounded-4xl border border-white/10 bg-white/4 p-7 shadow-[0_0_80px_rgba(0,0,0,0.28)] backdrop-blur-sm md:p-8">
            <div className="border-b border-white/10 pb-6">
              <p className="text-xs uppercase tracking-[0.3em] text-white/40">Cronograma</p>
            ...
            <p className="mt-6 text-xs uppercase tracking-[0.3em] text-white/40">Que incluye</p>
            <h2 className="mt-4 text-2xl font-semibold text-white">Incluido en {servicePackage.title}</h2>
        ...
        <section className="mt-14 grid gap-6 lg:grid-cols-2">
          <div className="rounded-4xl border border-white/10 bg-white/4 p-7 shadow-[0_0_80px_rgba(0,0,0,0.28)]">
            <h2 className="text-2xl font-semibold text-white">Para quien es</h2>
            <ul className="mt-6 space-y-4">
          ...
          <div className="rounded-4xl border border-white/10 bg-white/4 p-7 shadow-[0_0_80px_rgba(0,0,0,0.28)]">
            <h2 className="text-2xl font-semibold text-white">Como lo llevamos adelante</h2>
            <ul className="mt-6 space-y-4">
        ...
        <section className="mt-14">
          <ProjectBookingFlow
            contextLabel={`${servicePackage.title} valoracion`}
